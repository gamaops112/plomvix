...
func requireScalar(k Key) (Field, error) {
	if len(k.fields) == 0 && len(k.data) == 0 {
		return Field{}, ErrInvalidKey
	}
	if len(k.fields) != 1 {
		return Field{}, ErrInvalidKey
	}
...
func DecodeUint64(k Key) (uint64, error) {
	...
	if err := validateField(k.data, f); err != nil {
		return 0, err
	}
...
func ParseKey(data []byte, kinds []Kind) (Key, error) {
	...
	if pos != len(data) && kinds[len(kinds)-1] != KindBytes {
		return Key{}, ErrInvalidKey
	}
...
func EncodeStorageComposite(fields ...any) (Key, error) {
	...
	for _, f := range fields {
		...
		case uint64:
			...
			off := len(d)
			cb := make([]byte, 8)
			binary.BigEndian.PutUint64(cb, v)
			d = append(d, cb...)
