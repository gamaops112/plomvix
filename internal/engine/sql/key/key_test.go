...
func TestKindConstants(t *testing.T) {
	if key.KindUint64 != 1 { t.Errorf("KindUint64 = %d", key.KindUint64) }
	if key.KindInt64 != 2  { t.Errorf("KindInt64 = %d", key.KindInt64) }
	if key.KindString != 3 { t.Errorf("KindString = %d", key.KindString) }
	if key.KindBytes != 4  { t.Errorf("KindBytes = %d", key.KindBytes) }
}

func TestErrorSentinels(t *testing.T) {
	if key.ErrNullByteInString.Error() != "sql/key: string contains null byte" { t.Error("ErrNullByteInString") }
	if key.ErrUnsupportedType.Error() != "sql/key: unsupported field type" { t.Error("ErrUnsupportedType") }
	if key.ErrUnsupportedSortType.Error() != "sql/key: type not allowed in sort composite" { t.Error("ErrUnsupportedSortType") }
	if key.ErrInvalidKey.Error() != "sql/key: invalid key" { t.Error("ErrInvalidKey") }
	if key.ErrKindMismatch.Error() != "sql/key: kind mismatch" { t.Error("ErrKindMismatch") }
	if key.ErrNotComposite.Error() != "sql/key: key is not composite" { t.Error("ErrNotComposite") }
}

func TestZeroKey(t *testing.T) {
	var k key.Key
	if k.Bytes() != nil { t.Error("zero Key Bytes should be nil") }
	if k.Compare(k) != 0 { t.Error("zero Key Compare(self) should be 0") }
	if k.Fields() != nil { t.Error("zero Key Fields should be nil") }
}
...
func TestUint64RoundTrip(t *testing.T) {
	for _, v := range []uint64{0, 1, math.MaxUint64, math.MaxUint64 / 2} {
		...
		if err != nil || got != v { t.Errorf("uint64 %d: got %d, err %v", v, got, err) }
	}
}
...
func TestInt64RoundTrip(t *testing.T) {
	for _, v := range []int64{0, 1, -1, math.MinInt64, math.MaxInt64} {
		...
		if err != nil || got != v { t.Errorf("int64 %d: got %d, err %v", v, got, err) }
	}
}
...
func TestDecodeZeroKey(t *testing.T) {
	...
	if !errors.Is(err, key.ErrInvalidKey) { t.Errorf("DecodeUint64(zero): %v", err) }
	_, err = key.DecodeInt64(k)
	if !errors.Is(err, key.ErrInvalidKey) { t.Errorf("DecodeInt64(zero): %v", err) }
}

func TestDecodeKindMismatch(t *testing.T) {
	_, err := key.DecodeUint64(key.EncodeInt64(1))
	if !errors.Is(err, key.ErrKindMismatch) { t.Errorf("got %v", err) }
...
func TestStringRoundTrip(t *testing.T) {
	for _, s := range []string{"", "hello", "plomvix", "a\x01b"} {
		...
		if err != nil || got != s { t.Errorf("string %q: got %q, err %v", s, got, err) }
	}
}
...
func TestStringSortOrder(t *testing.T) {
	...
	for _, s := range []string{"", "a", "aa", "b", "z"} {
		k, _ := key.EncodeString(s); ks = append(ks, k)
	}
...
func TestParseKeyUint64(t *testing.T) {
	...
	if err != nil || v != 42 { t.Errorf("got %d, err %v", v, err) }
}
...
func TestParseKeyString(t *testing.T) {
	...
	if err != nil || s != "hi" { t.Errorf("got %q, err %v", s, err) }
}

func TestParseKeyErrors(t *testing.T) {
	if _, err := key.ParseKey(nil, nil); !errors.Is(err, key.ErrInvalidKey) { t.Error("empty kinds") }
	if _, err := key.ParseKey([]byte{0,0,0}, []key.Kind{key.KindUint64}); !errors.Is(err, key.ErrInvalidKey) { t.Error("too short") }
	if _, err := key.ParseKey([]byte("no_null"), []key.Kind{key.KindString}); !errors.Is(err, key.ErrInvalidKey) { t.Error("no null terminator") }
	if _, err := key.ParseKey([]byte{0,0,0,0,0,0,0,0, 0x99}, []key.Kind{key.KindUint64}); !errors.Is(err, key.ErrInvalidKey) { t.Error("leftover bytes") }
}
...
func TestParseKeyBytesLastOnly(t *testing.T) {
	...
	if err != nil || !bytes.Equal(b, []byte{1, 2, 3}) { t.Error("bytes round-trip") }
	// KindBytes in non-last position
	_, err = key.ParseKey(raw, []key.Kind{key.KindBytes, key.KindUint64})
	if !errors.Is(err, key.ErrInvalidKey) { t.Errorf("got %v", err) }
}
...
func TestParseKeyCopySafety(t *testing.T) {
	...
	b := k.Bytes(); b[0] = 0xFF
	b2 := k.Bytes()
	if b2[0] == 0xFF { t.Error("Bytes() mutation leaked") }
	fs := k.Fields(); fs[0].Offset = 999
	fs2 := k.Fields()
	if fs2[0].Offset == 999 { t.Error("Fields() mutation leaked") }
}
...
func TestParseKeyCompare(t *testing.T) {
	...
	if k1.Compare(k2) >= 0 { t.Error("1 should sort before 2") }
}
...
func TestSortCompositeRoundTrip(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if vals[0].(uint64) != 1 || vals[1].(uint64) != 2 { t.Error("round trip") }

	k2, _ := key.EncodeSortComposite(int64(-1), int64(0))
	vals, _ = key.DecodeSortComposite(k2)
	if vals[0].(int64) != -1 || vals[1].(int64) != 0 { t.Error("int64 round trip") }
}
...
func TestSortCompositeRejectsStrings(t *testing.T) {
	...
	if !errors.Is(err, key.ErrUnsupportedSortType) { t.Errorf("got %v", err) }
	_, err = key.EncodeSortComposite([]byte{1})
	if !errors.Is(err, key.ErrUnsupportedSortType) { t.Errorf("got %v", err) }
	_, err = key.EncodeSortComposite(true)
	if !errors.Is(err, key.ErrUnsupportedType) { t.Errorf("got %v", err) }
}

func TestDecodeZeroSortComposite(t *testing.T) {
	var k key.Key
	_, err := key.DecodeSortComposite(k)
	if !errors.Is(err, key.ErrNotComposite) { t.Errorf("got %v", err) }
}
...
func TestParseKeySortComposite(t *testing.T) {
	...
	vals, _ := key.DecodeSortComposite(k2)
	if vals[0].(uint64) != 1 || vals[1].(int64) != -1 { t.Error("parse+decode") }
}
...
func TestStorageCompositeNoNullTerminator(t *testing.T) {
	...
	if k.Fields()[0].Length != len("hello") { t.Error("storage composite string should not include null terminator") }
}
...
func TestStorageCompositeErrors(t *testing.T) {
	...
	if !errors.Is(err, key.ErrNullByteInString) { t.Errorf("got %v", err) }
}
...
func TestSortVsStorageByteDistinct(t *testing.T) {
	...
	if bytes.Equal(sort.Bytes(), storage.Bytes()) { t.Error("formats must differ") }
}
...
func TestScalarRejectsStorageComposite(t *testing.T) {
	...
	if !errors.Is(err, key.ErrInvalidKey) { t.Errorf("DecodeString: %v", err) }
	_, err = key.DecodeBytes(sc)
	if !errors.Is(err, key.ErrInvalidKey) { t.Errorf("DecodeBytes: %v", err) }
}
...
func TestDocumentation(t *testing.T) {
	...
	if err != nil { t.Fatalf("docs/sql_key.md not found: %v", err) }
	c := string(data)
	for _, s := range []string{
		...
		if !bytes.Contains([]byte(c), []byte(s)) { t.Errorf("missing: %q", s) }
	}
}
...
func must(k key.Key, err error) key.Key { if err != nil { panic(err) }; return k }
